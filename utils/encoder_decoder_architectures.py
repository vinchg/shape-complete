...
def construct_encoder_block(conf, inputs, name, down_outputs, first=False, last=False):
    ...
    conv1 = tf.layers.conv3d(inputs, num_outputs, conf.conv_size, strides=stride, name=name + '/conv1', activation=None,
                             use_bias=True)
...
def construct_decoder_deconv_block(conf, inputs, name, down_inputs, first = False, last = False):
    ...
    deconv1 = tf.layers.conv3d_transpose(cat,num_outputs,conf.conv_size if first else stride, strides=stride, name = name + '/deconv1', activation=None, use_bias=True)
...
def construct_decoder_pdcn_block(conf, inputs, name, down_inputs, first = False, last = False):
    num_outputs = 1 if last else int(inputs.shape[conf.channel_axis].value / 2)
    cat = tf.concat([inputs, down_inputs], conf.channel_axis, name = name + '/concat')
